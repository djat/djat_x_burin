...
9. [Provenance, Derivatives, and Persistence](#9-provenance-derivatives-and-persistence) - Aqua: §9.6 (MVP), §9.7 (multi-user lineage & merge framing), Burin: §9.9 (presence substrate)
...
| Presence binding optional | **Presence attestation** (Burin seals) travels with the run when templates use `burin_*` agents - see [§9.9](#99-burin-protocol--spatiotemporal-presence-substrate) |
...
| **Burin Protocol presence substrate** (`burin_*` agents, `burin_seal` artifacts, dual-substrate bundles) | ⚠ Partial - reference impl in `djat_x_burin`; [§9.9](#99-burin-protocol--spatiotemporal-presence-substrate) |
| **`substrate_license_ref` on `PathwayLicenseTerms`** (orchestration vs kernel license split) | ❌ Planned - proposed patch N5 in `burin-pathways` bundle |
| **Spatiotemporal gate (R6 extension)** - phase transitions bound to rHEALPix region | ❌ Planned - proposed patch N7 |
...
### 9.9 Burin Protocol - spatiotemporal presence substrate

**Burin** is a complementary cryptographic substrate to Aqua: where Aqua attests **orchestration lineage** (which recipe ran, who forked it, which revision shaped the work), Burin attests **presence** (what commitment was made, at which equal-area cells, at which time - verifiable offline, consensus-free). The Burin project lives at [`Burin/`](Burin/) in the `djat_x_burin` workspace; Pathways formalization is in [`burin-pathways/`](burin-pathways/).

English overloads *attestation* again here. This document uses:

| Sense | Substrate | What it binds |
|---|---|---|
| **Orchestration attestation** | Aqua Protocol | Template/run revision history, fork lineage, genesis seeds |
| **Presence attestation** | Burin Protocol | Seal over `commitment ‖ [cell] ‖ [time]`; witness append-only logs; fraud proofs |

Both can attach to the same `PathwayRun` without merging into one super-hash - the **dual-attestation sandwich** pattern ([`Presence.Bundle.DualAttestation@v1`](burin-pathways/pathways/Presence.Bundle.DualAttestation.v1.yaml)) exports hash-linked cross-references only.

#### 9.9.1 Burin concepts mapped to Pathways

| Burin concept | Pathways encoding | Source |
|---|---|---|
| **Seal primitive** | Step artifact `burin_seal` on `_step_artifacts[order]` | [`Burin/KERNEL.md`](Burin/KERNEL.md) |
| **Coverage / value dual-root** | `input_contract` fields `coverage_root` vs `value_root` (mutually exclusive commit modes) | [`Burin/COMMITMENT.md`](Burin/COMMITMENT.md) |
| **Witness attestation log** | Append-only `PathwayRunSource` edges with `role: presence_witness` | [`Burin/ATTESTATION_MODEL.md`](Burin/ATTESTATION_MODEL.md) |
| **Fraud proofs** | Artifact `burin_fraud_proof`; triggers R5 halt when `gate_profile.R5.halt_on_fraud` | [`Burin/burin/coverage/fraud.py`](Burin/burin/coverage/fraud.py) |
| **Trust card** | Cold-start artifact parallel to practice profile (R1) - witness key-set + DGGS params | [`Burin/SCOPE.md`](Burin/SCOPE.md) design law 3 |
| **ZK bounded disclosure** | Sub-pathway / step output `burin_zk_survey_proof` | [`Burin/zk/README.md`](Burin/zk/README.md) |
| **Degrade-to-paper** | Derivative step emits `spoken_seal_24w` + printable trust card | [`Burin/SCOPE.md`](Burin/SCOPE.md) §2 |

#### 9.9.2 Agents and templates

Burin-backed pathways use dedicated agents registered in the executor:

| Agent id | Role |
|---|---|
| `burin_canonicalize` | Polygon → equal-area rHEALPix fingerprint + cell list |
| `burin_presence` | Commit coverage, seal, witness attestation, fraud detection |
| `burin_zk` | Survey-effort and membership ZK proofs (bounded disclosure) |
| `burin_export` | Paper export, dual-attestation bundle assembly |

Seeded templates ship in [`burin-pathways/pathways/`](burin-pathways/pathways/) - flagship: `Presence.Survey.EffortProof@v1` (conservation / citizen-science effort proof without exact location leak).

#### 9.9.3 Dual licensing (orchestration vs kernel)

Burin kernel code is **PolyForm Noncommercial 1.0.0**; commercial kernel use requires a separate license from the substrate originator. Pathways `license_terms` on templates govern **orchestration recipes** only. The proposed `substrate_license_ref` field (patch N5) makes this split machine-readable:

```yaml
license_terms:
  type: revenue-share
  royalty_split: [...]   # pathway marketplace economics
...
Noncommercial research, education, and evaluation may run both kernel and templates freely. See [`burin-pathways/APPLICATION_PLAYBOOK.md`](burin-pathways/APPLICATION_PLAYBOOK.md) §4.

#### 9.9.4 Status (July 2026)
...
| Transect Trust UI (multi-party conservation + Trust Key) | ✅ [`transect-trust/`](../transect-trust/) - live: [209.46.125.56](http://209.46.125.56/) |
...
|---|---|---|---|
| **[Trust Key](trust-key/)** | Portable provenance root technique | [`trust-key/`](trust-key/) · [`burin-pathways/techniques/TRUST_KEY_TECHNIQUE.v1.md`](burin-pathways/techniques/TRUST_KEY_TECHNIQUE.v1.md) | local dev (5176) |
| **[Transect Trust](transect-trust/)** | Multi-party conservation quarter | [`transect-trust/`](transect-trust/) · [`burin-pathways/TRANSECT_TRUST_PLAYBOOK.md`](burin-pathways/TRANSECT_TRUST_PLAYBOOK.md) | [209.46.125.56](http://209.46.125.56/) |
...
All three apps share the [`backend/`](backend/) Pathways engine and Burin agents. Transect Trust adds Conservation templates and `/api/transect-trust/`; Trust Key adds `/api/trust-key/` and `Pathways.TrustKey.*` demos.
...
| **Burin substrate alignment - agents & artifacts** | `burin_canonicalize`, `burin_presence`, `burin_zk`, `burin_export`; `burin_seal` on `_step_artifacts` | Partial - reference impl [`djat_x_burin/backend/`](backend/) |
| **Burin substrate alignment - dual licensing** | `substrate_license_ref` on templates; `Pattern.BurinSubstrate.DualLicenseSplit` | Proposed N5; playbook shipped |
| **Burin substrate alignment - dual-attestation bundles** | `Presence.Bundle.DualAttestation@v1`; offline verify without merged super-hash | Partial - collaboration bundle H-BU1/H-BU5 |
| **Burin substrate alignment - spatiotemporal gate** | R6 extension binding phase transitions to public rHEALPix cells | Proposed N7 |
...
| **Orchestration attestation** | Aqua Protocol attestations over pathway templates and runs - *what workflow ran, under what lineage*. See [§9.6](#96-aqua-protocol-attestations-shipped-mvp). Distinct from presence attestation. |
| **Presence attestation** | Burin Protocol seals and witness logs - *what commitment, where (equal-area cells), when* - offline-verifiable, consensus-free. See [§9.9](#99-burin-protocol--spatiotemporal-presence-substrate). |
| **Burin seal** | Signed composite `H(DOMAIN ‖ commitment ‖ [cell] ‖ [time])` plus opening; degrades to 24 spoken words. Step artifact type `burin_seal`. |
| **Trust card** | Printable pre-distributed witness key-set + DGGS params; cold-start artifact parallel to practice profile for field verification. |
| **Dual-attestation sandwich** | Export pattern: Aqua run attestation + Burin seal chain in one bundle via hash-linked cross-references, not a merged super-hash. Template: `Presence.Bundle.DualAttestation@v1`. |
| **substrate_license_ref** (proposed) | Machine-readable pointer from pathway `license_terms` to underlying substrate license (e.g. Burin PolyForm NC); orchestration recipes licensable separately from kernel commercial terms. |
...
### A.8 Burin substrate integration provenance (July 2026)

| Field | Value |
|---|---|
| **Revision** | Architecture doc extended with [§9.9](#99-burin-protocol--spatiotemporal-presence-substrate) |
| **Date** | 2026-07-01 |
...
| **Live demo** | [Transect Trust](http://209.46.125.56/) on VPS |
